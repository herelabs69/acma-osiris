...
const getLayerGroups = (theme: 'core' | 'ghost') => {
  ...
  return [
  {
    ...
    color: '#1565C0',
    layers: [
      { key: 'sdk_sea', label: 'เส้นทางเดินเรือ', icon: Anchor, color: '#4FC3F7', dataKey: 'sdk_entities' },
      { key: 'sdk_ransomware', label: 'ฟีดแรนซัมแวร์', icon: AlertTriangle, color: '#D32F2F', dataKey: 'sdk_entities' },
    ],
  },
  {
    label: 'การบิน',
    ...
    color: flightCom,
    layers: [
      { key: 'flights', label: 'พาณิชย์', icon: Plane, color: flightCom, dataKey: 'commercial_flights' },
      { key: 'private', label: 'ส่วนตัว', icon: Plane, color: flightPriv, dataKey: 'private_flights' },
      { key: 'jets', label: 'เจ็ตส่วนตัว', icon: Plane, color: flightGov, dataKey: 'private_jets' },
      { key: 'military', label: 'ทหาร', icon: Shield, color: flightMil, dataKey: 'military_flights' },
    ],
  },
  {
    label: 'ทางทะเล',
    fullLabel: 'ทางทะเล & อวกาศ',
    color: '#26C6DA',
    layers: [
      { key: 'maritime', label: 'เรือ / กองทัพเรือ', icon: Ship, color: '#26C6DA', dataKey: 'maritime_ships,maritime_ports,maritime_chokepoints' },
      { key: 'satellites', label: 'ดาวเทียม', icon: Satellite, color: '#D4AF37', dataKey: 'satellites' },
    ],
  },
  {
    label: 'สอดส่อง',
    fullLabel: 'การสอดส่อง',
    color: '#7E57C2',
    layers: [
      { key: 'cctv', label: 'กล้อง CCTV', icon: Camera, color: '#7E57C2', dataKey: 'cameras' },
      { key: 'live_news', label: 'ฟีดข่าวสด', icon: Tv, color: '#EC407A', dataKey: 'live_feeds' },
    ],
  },
  {
    label: 'ภัยพิบัติ',
    fullLabel: 'ภัยธรรมชาติ',
    color: '#F9A825',
    layers: [
      { key: 'earthquakes', label: 'แผ่นดินไหว (24 ชม.)', icon: Activity, color: '#F9A825', dataKey: 'earthquakes' },
      { key: 'fires', label: 'ไฟกำลังลุก', icon: Flame, color: '#E65100', dataKey: 'fires' },
      { key: 'weather', label: 'สภาพอากาศรุนแรง', icon: CloudLightning, color: '#7E57C2', dataKey: 'weather_events' },
    ],
  },
  {
    label: 'ภัยคุกคาม',
    fullLabel: 'ภัยคุกคาม & โครงสร้าง',
    color: '#D32F2F',
    layers: [
      { key: 'infrastructure', label: 'โรงงานนิวเคลียร์', icon: Radiation, color: '#26A69A', dataKey: 'infrastructure' },
      { key: 'global_incidents', label: 'เหตุการณ์ทั่วโลก', icon: AlertTriangle, color: '#D32F2F', dataKey: 'gdelt' },
      { key: 'gps_jamming', label: 'การรบกวน GPS', icon: Radio, color: '#D32F2F', dataKey: 'gps_jamming' },
    ],
  },
  {
    label: 'เครือข่าย',
    fullLabel: 'ข่าวกรองเครือข่าย',
    color: '#D32F2F',
    layers: [

      { key: 'malware', label: 'มัลแวร์สด', icon: AlertTriangle, color: '#D32F2F', dataKey: 'malware_threats' },
    ],
  },
  {
    label: 'การแสดงผล',
    ...
    color: '#448AFF',
    layers: [
      { key: 'day_night', label: 'กลางวัน / กลางคืน', icon: Sun, color: '#448AFF', dataKey: '' },
      { key: 'terrain_3d', label: 'ภูมิประเทศ & อาคาร 3D', icon: Mountain, color: '#8D6E63', dataKey: '' },
    ],
  },
  ];
};
    ...
    >
      ...
      {setTheme && (
        <div className="mt-auto px-2 pt-6 pb-2 border-t border-[var(--border-primary)] flex flex-col items-center gap-3 relative z-50">
          <div className="text-[9px] font-mono tracking-[0.25em] text-[var(--text-secondary)]">โหมดโกสต์</div>
          <button
            onClick={() => setTheme(theme === 'core' ? 'ghost' : 'core')}
