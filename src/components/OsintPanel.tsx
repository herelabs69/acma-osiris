...
const TABS = [
  { id: 'scanner', label: 'สแกนพอร์ต', icon: Radar, placeholder: 'IP หรือ hostname', color: '#00E5FF' },
  { id: 'vuln', label: 'สแกนช่องโหว่', icon: Bug, placeholder: 'IP หรือ hostname', color: '#FF3D3D' },

  { id: 'dns', label: 'DNS', icon: Server, placeholder: 'ชื่อโดเมน', color: '#448AFF' },
  { id: 'whois', label: 'WHOIS', icon: FileText, placeholder: 'ชื่อโดเมน', color: '#FFD700' },
  { id: 'certs', label: 'ใบรับรอง', icon: Lock, placeholder: 'ชื่อโดเมน', color: '#E040FB' },
  { id: 'threats', label: 'ภัยคุกคาม', icon: AlertTriangle, placeholder: 'IP, โดเมน หรือ hash', color: '#FF9500' },
  { id: 'headers', label: 'เฮดเดอร์', icon: Code, placeholder: 'URL ที่ต้องการตรวจ', color: '#87CEEB' },
  { id: 'ssl', label: 'SSL/TLS', icon: Shield, placeholder: 'ชื่อโดเมน', color: '#76FF03' },
  { id: 'subdomains', label: 'ซับโดเมน', icon: Layers, placeholder: 'โดเมนที่ต้องการค้นหา', color: '#00BCD4' },
  { id: 'tech', label: 'ตรวจเทคโนโลยี', icon: Code, placeholder: 'URL ที่ต้องการระบุ', color: '#9C27B0' },
  { id: 'shodan', label: 'SHODAN IoT', icon: Network, placeholder: 'หมายเลข IP', color: '#FF3D3D' },
  { id: 'bgp', label: 'เส้นทาง BGP', icon: Globe, placeholder: 'IP หรือ ASN', color: '#00E5FF' },
  { id: 'mac', label: 'MAC แอดเดรส', icon: Fingerprint, placeholder: 'หมายเลข MAC', color: '#FFD700' },
  { id: 'phone', label: 'ข่าวกรองเบอร์', icon: Phone, placeholder: 'เบอร์โทร (เช่น +66...)', color: '#FF9500' },
  { id: 'leaks', label: 'ข้อมูลรั่วไหล', icon: ShieldAlert, placeholder: 'อีเมล', color: '#E040FB' },
  { id: 'github', label: 'สำรวจ GitHub', icon: Terminal, placeholder: 'ชื่อผู้ใช้ GitHub', color: '#87CEEB' },
  { id: 'sweep', label: 'สแกน IP', icon: Crosshair, placeholder: 'ใส่หมายเลข IP (เช่น 8.8.8.8)', color: '#FF3D3D' },
];
...
function OsintPanelInner({ isMobile, onSweepVisualize, onScanGeolocate }: OsintPanelProps) {
  ...
  const renderStructuredResults = () => {
    ...
    if (activeTab === 'scanner') {
      ...
      return (
        <div>
          <SectionHeader title="ข้อมูลโฮสต์" icon={Server} color="#00E5FF" />
    ...
    if (activeTab === 'vuln') {
      ...
      return (
        <div>
          <SectionHeader title="ประเมินช่องโหว่" icon={Bug} color="#FF3D3D" />
    ...
    if (activeTab === 'dns') {
      return (
        <div>
          <SectionHeader title="ระเบียน DNS" icon={Server} color="#448AFF" />
    ...
    if (activeTab === 'whois') {
      return (
        <div>
          <SectionHeader title="ข่าวกรอง WHOIS" icon={FileText} color="#FFD700" />
    ...
    if (activeTab === 'shodan') {
      return (
        <div>
          <SectionHeader title="ข่าวกรอง SHODAN IoT" icon={Network} color="#FF3D3D" />
    ...
    if (activeTab === 'bgp') {
      return (
        <div>
          <SectionHeader title="ข่าวกรองเส้นทาง BGP" icon={Globe} color="#00E5FF" />
    ...
    if (activeTab === 'mac') {
      return (
        <div>
          <SectionHeader title="ค้นหาผู้ผลิต MAC" icon={Fingerprint} color="#FFD700" />
    ...
    if (activeTab === 'phone') {
      return (
        <div>
          <SectionHeader title="ข่าวกรองเบอร์โทร" icon={Phone} color="#FF9500" />
    ...
    if (activeTab === 'github') {
      return (
        <div>
          <SectionHeader title="สำรวจ GitHub" icon={Terminal} color="#87CEEB" />
          ...
          {r.recent_repos?.length > 0 && (
            <div className="mt-2 p-2 border border-[#87CEEB]/20 bg-[#87CEEB]/5 rounded">
              <span className="text-[9px] font-mono text-[#87CEEB] block mb-1">รีโปล่าสุด</span>
    ...
    if (activeTab === 'leaks') {
      return (
        <div>
          <SectionHeader title="ตรวจข้อมูลรั่วไหล" icon={ShieldAlert} color="#E040FB" />
          ...
          {r.breached && r.data_exposed?.length > 0 && (
            <div className="mt-2 p-2 border border-[#E040FB]/30 bg-[#E040FB]/10 rounded">
              <span className="text-[10px] font-mono text-[#E040FB] font-bold mb-1 block">ข้อมูลที่เปิดเผย</span>
              <div className="flex flex-wrap gap-1">
    ...
    if (activeTab === 'certs') {
      ...
      return (
        <div>
          <SectionHeader title="ใบรับรอง (CT Logs)" icon={Lock} color="#E040FB" />
    ...
    if (activeTab === 'threats') {
      return (
        <div>
          <SectionHeader title="ข่าวกรองภัยคุกคาม" icon={AlertTriangle} color="#FF9500" />
    ...
    if (activeTab === 'ssl') {
      return (
        <div>
          <SectionHeader title="วิเคราะห์ SSL/TLS" icon={Shield} color="#76FF03" />
  ...
  const renderContent = () => (
    <div className="flex flex-col gap-2.5">
            ...
            <button key={tab.id} onClick={() => { 
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono font-bold tracking-[0.1em] text-[11px]" style={{ color: tab.color }}>สแกนทั่วโลก</span>
                </div>
            </button>
          ))}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {activeTab === 'scanner' && (
          <select value={scanType} onChange={e => setScanType(e.target.value)}
            ...
            <option value="quick">สแกนเร็ว</option><option value="deep">สแกนลึก</option><option value="ports">พอร์ตยอดนิยม 1000</option>
          </select>
        )}
        {(activeTab === 'sweep' || activeTab === 'vuln') && (
          <div className="flex items-center justify-between bg-[var(--bg-primary)]/60 border border-[var(--border-primary)] rounded-lg p-1">
            <span className="text-[9px] font-mono text-[var(--text-muted)] pl-2">ซับเน็ตมาสก์:</span>
      ...
      {sweepProgress && loading && (
        <div className="p-3 rounded-lg border border-[#FF3D3D]/30 bg-[#FF3D3D]/5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-mono tracking-wider text-[#FF3D3D]">กำลังสแกนซับเน็ต...</span>
          ...
          <div className="p-3 border-b border-[#2A2A28]">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                ...
                <div className="text-[8px] font-mono text-[#5C5A54] tracking-wider">พบอุปกรณ์</div>
              </div>
            </div>
            {/* Breakdown Bar */}
                      ...
                      {device.vulns.length > 0 && (
                        <div className="group relative flex items-center gap-1 cursor-help">
                          ...
                          <div className="absolute bottom-full left-0 mb-1 hidden group-hover:block z-50 p-2 bg-[#1A1A18] border border-[#FF3D3D50] rounded-md shadow-xl min-w-[140px] max-w-[220px] max-h-[150px] overflow-y-auto styled-scrollbar">
                            <div className="text-[8px] font-mono text-[#FF3D3D] mb-1 tracking-wider uppercase border-b border-[#FF3D3D30] pb-1">ช่องโหว่ที่พบ</div>
                  ...
                  <div className="border-t border-[#2A2A28]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-[#2A2A28]">
                      <div className="bg-[#0D0D0C] p-4">
                        <div className="text-[10px] font-mono text-[#5C5A54] tracking-widest uppercase mb-2">พอร์ตที่เปิด</div>
                      ...
                      <div className="bg-[#0D0D0C] p-4">
                        <div className="text-[10px] font-mono text-[#5C5A54] tracking-widest uppercase mb-2">ชื่อโฮสต์</div>
                        {device.hostnames.length > 0 ? (
                          <div className="flex flex-col gap-1">
                          ...
                          <span className="text-[11px] font-mono text-[#3A3A38]">ไม่มี reverse DNS</span>
                        )}
                      </div>
                    </div>
      ...
      {history.length > 0 && !results && (
        <div className="space-y-1">
          <span className="text-[9px] font-mono tracking-widest text-[var(--text-muted)]">สแกนล่าสุด</span>
      ...
      <div className="fixed top-4 bottom-4 right-4 w-[40vw] min-w-[600px] max-w-[800px] z-[999] glass-panel bg-[#0a0a09]/95 backdrop-blur-2xl border border-[var(--cyan-primary)]/40 rounded-xl flex flex-col overflow-hidden shadow-2xl shadow-[var(--cyan-primary)]/20 transition-all duration-300">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-secondary)] bg-[#111]">
          <div className="flex items-center gap-3">
            ...
            <span className="hud-text text-[16px] text-[var(--text-primary)]">ชุดเครื่องมือสำรวจ OSIRIS</span>
            <span className="gotham-tag gotham-tag--info" style={{ fontSize: '9px' }}>มุมมองขยาย</span>
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.6 }} className="glass-panel flex flex-col overflow-hidden pointer-events-auto shrink-0 h-[500px] max-h-[80vh] resize-y">
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.3)] hover:bg-[var(--hover-accent)] transition-colors">
        <button onClick={() => setExpanded(!expanded)} className="flex items-center gap-2 flex-1">
          ...
          <span className="hud-text text-[12px] text-[var(--text-primary)]">ชุดเครื่องมือสำรวจ</span>
