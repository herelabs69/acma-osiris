            ...
            <div className="flex flex-col border-b border-[var(--border-primary)] bg-black/60 relative z-10">
              ...
              <div className="flex items-center justify-between px-3 py-1 border-b border-white/5 text-[7px] font-mono tracking-[0.2em] text-[var(--text-muted)] bg-[var(--hover-accent)]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[var(--gold-primary)]">ลิงก์ปลอดภัย</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between px-3 md:px-4 py-2 md:py-3">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-3">
                  {streamType === 'jpg' && (
                    <button onClick={() => setRefreshKey(k => k + 1)} className="p-1.5 rounded-sm bg-white/5 border border-white/10 hover:bg-[var(--gold-primary)]/20 hover:border-[var(--gold-primary)] transition-all" title="รีเฟรชฟีด">
                      <RefreshCw className="w-3 h-3 text-[var(--text-secondary)] hover:text-[var(--gold-primary)]" />
                    </button>
                  )}
                  {camera.lat && camera.lng && (
                    <button onClick={() => onLocate?.(camera.lat, camera.lng)} className="p-1.5 rounded-sm bg-white/5 border border-white/10 hover:bg-[var(--gold-primary)]/20 hover:border-[var(--gold-primary)] transition-all" title="บินไปยังตำแหน่ง">
                      <MapPin className="w-3 h-3 text-[var(--text-secondary)] hover:text-[var(--gold-primary)]" />
                    </button>
                  )}
                  <button onClick={() => setFullscreen(!fullscreen)} className="hidden md:block p-1.5 rounded-sm bg-white/5 border border-white/10 hover:bg-[var(--text-primary)]/20 hover:border-[var(--text-primary)] transition-all" title="สลับเต็มจอ">
          ...
          <div className={`relative bg-[#020202] ${fullscreen ? 'flex-1 overflow-hidden' : 'aspect-video max-h-[35vh] md:max-h-none'}`}>
            ...
            {loading && !error && !externalOnly && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/90 z-30 backdrop-blur-sm">
                <div className="text-center">
                  ...
                  <span className="text-[9px] font-mono tracking-[0.25em]" style={{ color: 'var(--gold-primary)' }}>กำลังถอดรหัสฟีด...</span>
                </div>
              </div>
            )}
            ...
            {externalOnly ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 z-30 backdrop-blur-sm p-4 text-center">
                ...
                <p className="text-[10px] font-mono uppercase tracking-widest" style={{ color: 'var(--gold-primary)' }}>ฟีดเข้ารหัสปลอดภัย</p>
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/90">
                <div className="text-center">
                  ...
                  <span className="text-[9px] font-mono text-red-400 tracking-widest block mb-1">ฟีดไม่พร้อมใช้งาน</span>
                  <span className="text-[7px] font-mono text-[var(--text-muted)]">กล้องอาจออฟไลน์หรือถูกจำกัด</span>
            ...
            <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/5">
              <div className="flex gap-4">
                <div className="flex flex-col">
                  <span className="text-[6px] text-[var(--text-muted)] font-mono tracking-widest">ประเภทฟีด</span>
                  <span className="text-[8px] text-white font-mono tracking-widest uppercase">{streamType}</span>
                </div>
                <div className="flex flex-col border-l border-white/10 pl-4">
                  <span className="text-[6px] text-[var(--text-muted)] font-mono tracking-widest">สถานะ</span>
                  <span className="text-[8px] text-[var(--alert-green)] font-mono tracking-widest">ทำงาน / กำลังบันทึก</span>
                </div>
              </div>
